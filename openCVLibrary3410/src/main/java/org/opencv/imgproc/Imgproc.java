...
public class Imgproc {
    ...
    /**
     * activity_image_capturer function for all demosaicing processes
     *
     ...
     * @param axes Half of the size of the ellipse activity_image_capturer axes.
     ...
     * @param axes Half of the size of the ellipse activity_image_capturer axes. See #ellipse for details.
     ...
     * rotation-invariant template_card_1 matching, for object tracking and so forth.
     * @param src Source image
     * @param dst Destination image. It will have same size and type as src.
    ...
    /**
     * Compares a template_card_1 against overlapped image regions.
     *
     * The function slides through image , compares the overlapped patches of size \(w \times h\) against
     * templ using the specified method and stores the comparison results in result . Here are the formulae
     * for the available comparison methods ( \(I\) denotes image, \(T\) template_card_1, \(R\) result ). The summation
     * is done over template_card_1 and/or the image patch: \(x' = 0...w-1, y' = 0...h-1\)
     *
     * After the function finishes the comparison, the best matches can be found as global minimums (when
     * #TM_SQDIFF was used) or maximums (when #TM_CCORR or #TM_CCOEFF was used) using the
     * #minMaxLoc function. In case of a color image, template_card_1 summation in the numerator and each sum in
     * the denominator is done over all of the channels and separate mean values are used for each channel.
     * That is, the function can take a color template_card_1 and a color image. The result will still be a
     * single-channel image, which is easier to analyze.
     *
     * @param image Image where the search is running. It must be 8-bit or 32-bit floating-point.
     * @param templ Searched template_card_1. It must be not greater than the source image and have the same
     * data type.
     * @param result Map of comparison results. It must be single-channel 32-bit floating-point. If image
     * is \(W \times H\) and templ is \(w \times h\) , then result is \((W-w+1) \times (H-h+1)\) .
     * @param method Parameter specifying the comparison method, see #TemplateMatchModes
     * @param mask Mask of searched template_card_1. It must have the same datatype and size with templ. It is
